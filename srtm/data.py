...
class GeoElevationData:
    ...
    def __init__(self, srtm1_files, srtm3_files, swbd_files, leave_zipped=False,
                 file_handler=None):
        self.srtm1_files = srtm1_files
        self.srtm3_files = srtm3_files
        self.swbd_files = swbd_files
    ...
    def get_elevation(self, latitude, longitude, approximate=None):
        ...
        # thp: disable swbd file download
        # self.get_swbd_file(float(latitude), float(longitude))
    ...
    def get_swbd_file(self, latitude, longitude):
        """
        If the file can't be found -- it will be retrieved from the server.
        """
        # file_name = self.get_file_name(latitude, longitude)
        file_name = self.get_swbd_file_name(latitude, longitude)
        if not file_name:
            return None
    ...
    def retrieve_or_load_file_data(self, file_name):
        ...
        elif (file_name in self.swbd_files):
            url = self.swbd_files[file_name]
        ...
        if self.leave_zipped or file_name in self.swbd_files:
    ...
    def get_swbd_file_name(self, latitude, longitude):
        # Decide the file name:
        if latitude >= 0:
            north_south = 'n'
        else:
        ...
        file_name = '%s%s%s%s' % (east_west, str(int(abs(mod_math.floor(longitude)))).zfill(3), north_south, str(int(abs(mod_math.floor(latitude)))).zfill(2))

        if not (file_name in self.swbd_files):
    ...
    # def get_swbd_real_file_name(self, latitude, longitude):
    #     # Decide the file name:
    #     if latitude >= 0:
    #         north_south = 'n'
    #     else:
    ...
    #     return self.swbd_files[file_name].split('/')[-1]
...
class GeoElevationFile:
    ...
    def approximation(self, latitude, longitude):
        """
        Dummy approximationGeoElevationFile with nearest points. The nearest the neighbour the
