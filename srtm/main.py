...
DEFAULT_SWBD_LIST_JSON = package_location + mod_os.sep + 'swbd.json'
...
def get_data(srtm1=True, srtm3=True, leave_zipped=False, file_handler=None,
    ...
    swbd_files = _get_swbd_urls_json(use_included_urls, file_handler)
    ...
    return mod_data.GeoElevationData(srtm1_files, srtm3_files, swbd_files, file_handler=file_handler,
...
def _get_swbd_urls_json(use_included_urls, file_handler):
    if use_included_urls:
        with open(DEFAULT_SWBD_LIST_JSON, 'r') as f:
            return mod_json.loads(f.read())['swbd']
