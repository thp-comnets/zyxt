...
int MCF::getNodeLocalIndex(vector<long> nonRemovedNodes, long node){
    for(int i = 0; i < nonRemovedNodes.size(); i++){
        if(nonRemovedNodes[i] == node){
            return i;
        }
    }
    return -1;
...
void MCF::initializePreliminaryData() {
    ...
    while(fin >> nodeIndex){
        this->totalNonRemovedPoints++;
        //this->nonRemovedPoints[nodeIndex] = true;
        this->nonRemovedNodes.push_back(nodeIndex);
    }
    fin.close();
    ...
    if (!this->inputJson.empty()){
        ...
        for (json::iterator it = inputJson["nodes"].begin(); it != inputJson["nodes"].end(); ++it) {
            if (it[0]["nodeProperty"]["type"] == "source")
            {
                ...
                int localSourceIndex = this->getNodeLocalIndex(this->nonRemovedNodes, source);

                this->sourceMap[localSourceIndex] = int(it[0]["nodeProperty"]["capacity"]);
                //cout << localSourceIndex << endl;
            } else {
                int sinkNode = int(it[0]["node"]) - 1;
                int localSinkIndex = this->getNodeLocalIndex(this->nonRemovedNodes, sinkNode);
                int capacity = int(it[0]["nodeProperty"]["capacity"]) * -1;
                this->sinksMap[localSinkIndex] += capacity;
                //cout << localSinkIndex << endl;
...
double MCF::calculateEquipmentCostOnly(const SimpleMinCostFlow &minCostFlow) {
    ...
    for(int i = 0; i < minCostFlow.NumArcs(); i++) {
        if (minCostFlow.Flow(i) > 1) {
            Equipment equipment = this->getBestDevice(this->allCoordinates[ this->nonRemovedNodes[minCostFlow.Tail(i)] ], this->allCoordinates[ this->nonRemovedNodes[minCostFlow.Head(i)] ], minCostFlow.Flow(i));
...
void MCF::writeJSONOutput(const SimpleMinCostFlow &minCostFlow, int iter) {
    ...
    for(int i = 0; i < minCostFlow.NumArcs(); i++){
        if(minCostFlow.Flow(i) > 0){
            ...
            Edge edge;
            Equipment equipment = this->getBestDevice(this->allCoordinates[this->nonRemovedNodes[minCostFlow.Tail(i)]], this->allCoordinates[ this->nonRemovedNodes[minCostFlow.Head(i)] ], minCostFlow.Flow(i));
            output["edges"][outputCounter]["nodes"] = json::array({this->nonRemovedNodes[minCostFlow.Tail(i)], this->nonRemovedNodes[minCostFlow.Head(i)]});
            ...
            double distance = calculateDistance(this->allCoordinates[ this->nonRemovedNodes[minCostFlow.Tail(i)] ], this->allCoordinates[ this->nonRemovedNodes[minCostFlow.Head(i)] ]);
            ...
            nodes[this->nonRemovedNodes[minCostFlow.Head(i)] ] = minCostFlow.Supply(minCostFlow.Head(i));
            nodes[this->nonRemovedNodes[minCostFlow.Tail(i)] ] = minCostFlow.Supply(minCostFlow.Tail(i));
...
void MCF::optimizeMinSolution(int iterCount) {
    ...
    do{
        ...
        for(auto it = this->edgesKept.begin(); it != this->edgesKept.end(); ++it) {
            ...
            unsigned long long int totalNodes = (unsigned long long int) this->rows * this->cols;
            for (long i = 0; i < this->nonRemovedNodes.size(); i++) {
                for (long j = 0; j < this->nonRemovedNodes.size(); j++) {

                    if (getBit(this->nonRemovedNodes[i] * totalNodes + this->nonRemovedNodes[j])) {
                        double distance = this->calculateDistance(this->allCoordinates[ this->nonRemovedNodes[i] ], this->allCoordinates[ this->nonRemovedNodes[j] ]);
                        Equipment equipment = this->getBestDevice( this->allCoordinates[ this->nonRemovedNodes[i] ], this->allCoordinates[ this->nonRemovedNodes[j] ], this->maximumCapacity );
                        if (equipment.id == -1) {
                            continue;
                        }
                        double unitCost = this->getEdgeCost(distance); //ceil((double)equipment.totalPairCost / (double)equipment.throughput);
...
void MCF::furtherOptimizeMinSolution(int iterCount) {
    ...
    do{
        ...
        for(auto it = sortedKeptEdgeList.begin(); it != sortedKeptEdgeList.end(); ++it) {
            ...
            if (this->isEdgeWithLeafNode(it->first)) {
               cout << "This is skipped in part-2 of phase 2: " << it->first << "::" << it->second.passedFlow << endl;
                continue;
            }
            //it->second.isChecked = true;
